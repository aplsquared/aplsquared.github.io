...
function addToIDB(size) {
  ...
  return idbKV.set(key, data)
    .then(() => {
      console.log(`Added to IndexedDB: '${key}'`);
    })
    .catch((err) => {
...
function addToLocalStorage() {
  const key = `local_${Date.now().toString()}`;
  try {
    localStorage.setItem(key, getStr());
    console.log(`Added to Local Storage: '${key}'`);
  } catch (ex) {
...
function addToSessionStorage() {
  const key = `session_${Date.now().toString()}`;
  try {
...
/* Clear All Storage */
document.getElementById('buttClear').addEventListener('click', clearAll);
document.getElementById('buttClearIDB').addEventListener('click', clearIDB);
document.getElementById('buttClearCaches').addEventListener('click', clearCacheStorage);
document.getElementById('buttClearLocal').addEventListener('click', clearLocal);
document.getElementById('buttClearSession').addEventListener('click', clearSession);
